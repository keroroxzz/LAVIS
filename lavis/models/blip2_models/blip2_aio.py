...
class Blip2AIO(Blip2Base):
    ...
    def generate_old(
        ...
        if image.dim() == 5-dim_offset:
            ...
            for j in range(image.size(2)):
                this_frame = image[:,:,j,:,:]
                with self.maybe_autocast():
                    frame_embeds = self.ln_vision(self.visual_encoder(this_frame, beg_layer=beg_layer))
    ...
    def generate(
        ...
        """
        The input image could be a single image or a video.
        single image: BxCxHxW
        video: BxTxCxHxW
        """
    ...
    def predict_class_old(
        ...
        if type(candidates[0]) == list:
            ...
            for i in range(samples["image"].size(0)):
                ...
                this_result = self._predict_class_old(this_sample, candidates[i], n_segments, beg_layer=beg_layer)
        ...
        return self._predict_class_old(samples, candidates, n_segments, beg_layer=beg_layer)

    @torch.no_grad()
    def _predict_class_old(
        self,
        samples,
        candidates,
        n_segments=1,
